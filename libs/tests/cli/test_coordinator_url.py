...
class TestCommandGroupsTargetLoggedInCoordinator:
    ...
    def test_project_set_uses_saved_coordinator(self):
        """`set` resolves the Coordinator the same way `list` does; it looks the
        project up first, so it must query the logged-in Coordinator too."""
        with (
            patch(
                "arcade_cli.utils.get_saved_coordinator_url",
                return_value=DEDICATED_COORDINATOR_URL,
            ...
            result = CliRunner().invoke(project_module.app, ["set", "proj_1"])

        assert result.exit_code == 0, result.output
        fetch.assert_called_once_with(DEDICATED_COORDINATOR_URL, "org_1")

    def test_org_set_uses_saved_coordinator(self):
        with (
            patch(
                "arcade_cli.utils.get_saved_coordinator_url",
                return_value=DEDICATED_COORDINATOR_URL,
            ...
            result = CliRunner().invoke(org_module.app, ["set", "org_1"])
