...
def parse_date_range_for_relative_terms(date_str: str) -> tuple[datetime | None, datetime | None]:
...
def build_issue_filter(
    ...
    if search_query:
        ...
        if len(keywords) == 0:
            ...
            desc_conditions = [
                {"description": {"containsIgnoreCase": keyword}} for keyword in keywords
            ]
            label_conditions = [
...
async def resolve_cycle_by_relative_reference(
    context: ToolContext, cycle_reference: str, team_id: str, client: LinearClient | None = None
) -> dict[str, Any] | None:
    ...
    cycles_with_dates = [
        (cycle, parse_cycle_date(cycle)) for cycle in cycles if parse_cycle_date(cycle) is not None
    ]
...
async def _auto_transition_backlog_to_todo(
    ...
    is_backlog_state = (
        ...
        or any(backlog_name in current_state_name for backlog_name in backlog_state_names)
    )
...
async def _create_team_specific_label(
    ...
    try:
        ...
        if "duplicate" in error_message or "already exists" in error_message:
            ...
            raise ToolExecutionError(f"Failed to create label '{label_name}' for team: {e!s}")
