...
from arcade_linear.constants import PRIORITY_NAME_TO_VALUE, TIME_RANGE_MAPPINGS
...
def parse_date_range_for_relative_terms(date_str: str) -> tuple[Optional[datetime], Optional[datetime]]:
...
def is_relative_date_range_term(date_str: str) -> bool:
    ...
    relative_range_terms = [
        "last week", "past week", "this week", "current week",
        "last month", "past month", "this month", "current month", 
        "yesterday", "today",
        "last year", "past year", "this year", "current year"
    ]
    
    return date_str_lower in relative_range_terms


def validate_date_format(field_name: str, date_str: Optional[str]) -> None:
...
def normalize_priority(priority: Union[str, int, None]) -> Optional[int]:
    ...
    if isinstance(priority, int):
        if priority in [0, 1, 2, 3, 4]:
            ...
            raise ToolExecutionError(f"Invalid priority value: {priority}. Must be 0-4.")
...
def build_issue_filter(
    ...
    if search_query:
        ...
        common_words = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "from", "up", "about", "into", "through", "during", "before", "after", "above", "below", "over", "under", "again", "further", "then", "once", "here", "there", "when", "where", "why", "how", "all", "any", "both", "each", "few", "more", "most", "other", "some", "such", "no", "nor", "not", "only", "own", "same", "so", "than", "too", "very", "can", "will", "just", "should", "now"}
        ...
        if len(keywords) == 0:
            ...
            title_conditions = [{"title": {"containsIgnoreCase": keyword}} for keyword in keywords]
            desc_conditions = [{"description": {"containsIgnoreCase": keyword}} for keyword in keywords]
            label_conditions = [{"labels": {"some": {"name": {"containsIgnoreCase": keyword}}}} for keyword in keywords]
            ...
            single_keyword_conditions = title_conditions + desc_conditions + label_conditions
...
async def resolve_issue_identifier_to_uuid(context: ToolContext, issue_identifier: str) -> Optional[str]:
...
async def resolve_issue_with_team_info(context: ToolContext, issue_identifier: str) -> Optional[Dict[str, Any]]:
...
async def get_default_team_fallback(context: ToolContext) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # If we can't get teams, return None
        pass
    
    return None


async def resolve_team_by_name(context: ToolContext, team_name: str) -> Optional[Dict[str, Any]]:
...
async def resolve_user_by_email_or_name(context: ToolContext, identifier: str) -> Optional[Dict[str, Any]]:
...
async def resolve_workflow_state_by_name(context: ToolContext, state_name: str, team_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if len(exact_matches) == 1:
        ...
        raise ToolExecutionError(
            f"Multiple workflow states found with name '{state_name}': {', '.join(state_names)}. "
            f"Please specify the state more precisely."
        )
    ...
    if len(states) == 1:
        ...
        raise ToolExecutionError(
            f"Multiple workflow states found containing '{state_name}': {', '.join(state_names)}. "
            f"Please specify the state more precisely."
        )
...
async def resolve_project_by_name(context: ToolContext, project_name: str) -> Optional[Dict[str, Any]]:
...
async def resolve_projects_by_name(context: ToolContext, project_name: str) -> List[Dict[str, Any]]:
    ...
    for project in projects:
        ...
        if normalized_project == normalized_search:
            variation_matches.append(clean_project_data(project))
        # Try space to underscore
        elif project_name_lower == space_to_underscore:
            variation_matches.append(clean_project_data(project))
    ...
    for project in projects:
        ...
        if (search_lower in project_name_lower and 
            len(search_lower) >= 3 and  # Minimum 3 characters for partial match
            len(project_name_lower) <= len(search_lower) * 2):  # Project name not more than 2x longer
            partial_matches.append(clean_project_data(project))
    
    return partial_matches


async def resolve_cycle_by_name(context: ToolContext, cycle_name: str, team: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
async def resolve_labels_by_names(context: ToolContext, label_names: List[str]) -> List[Dict[str, Any]]:
    ...
    for label_name in label_names:
        ...
        if not labels:
            raise ToolExecutionError(f"Label '{label_name}' not found.")
        ...
        if len(exact_matches) == 1:
            ...
            raise ToolExecutionError(
                f"Multiple labels found with name '{label_name}': {', '.join(label_names_list)}. "
                f"Please specify the label more precisely."
            )
        else:
            # Look for partial matches
            if len(labels) == 1:
                ...
                raise ToolExecutionError(
                    f"Multiple labels found containing '{label_name}': {', '.join(label_names_list)}. "
                    f"Please specify the label more precisely."
                )
...
async def resolve_labels_with_autocreate(
    context: ToolContext, 
    label_names: List[str], 
    team_id: Optional[str] = None
) -> List[Dict[str, Any]]:
    """Resolve label names to label data, automatically creating team-specific labels as needed
...
async def _auto_transition_backlog_to_todo(
    context: ToolContext,
    current_issue: Dict[str, Any],
    update_input: Dict[str, Any],
    team_id: str,
    user_provided_status: Optional[str]
) -> None:
    ...
    is_backlog_state = (
        current_state_name in backlog_state_names or
        current_state_type in backlog_state_types or
        any(backlog_name in current_state_name for backlog_name in backlog_state_names)
    )
    ...
    try:
        ...
        todo_state_names = ["to do", "todo", "ready", "ready for dev", "ready to start", "open"]
        todo_state = None
        ...
        for state in workflow_states["nodes"]:
            ...
            if state_type == "started" and any(name in state_name for name in todo_state_names):
                todo_state = state
                break
...
async def _create_team_specific_label(
    ...
    try:
        ...
        if create_result.get("success") and create_result.get("label"):
            ...
            raise ToolExecutionError(f"Failed to create label '{label_name}' for team")
        ...
        if "duplicate" in error_message or "already exists" in error_message:
            ...
            try:
                ...
                for label in all_updated_labels:
                    if (label["name"].lower() == label_name_lower and 
                        (not label.get("team") or label.get("team", {}).get("id") == team_id)):
                        resolved_labels.append(label)
                        return
            ...
            raise ToolExecutionError(
                f"Failed to create label '{label_name}' for team: {str(e)}"
            )
...
async def resolve_labels_read_only(
    ...
    if missing_labels:
        available_label_names = [label["name"] for label in available_labels[:10]]  # Show up to 10 available labels
        if team_id:
            team_info = f"in this team"
        else:
            team_info = f"in the workspace"
...
def clean_issue_data(issue_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if issue_data.get("relations") and issue_data["relations"].get("nodes"):
        cleaned["relations"] = [clean_relation_data(relation) for relation in issue_data["relations"]["nodes"]]
    
    # Handle comments if present
    if issue_data.get("comments") and issue_data["comments"].get("nodes"):
        cleaned["comments"] = [clean_comment_data(comment) for comment in issue_data["comments"]["nodes"]]
    
    # Handle attachments if present  
    if issue_data.get("attachments") and issue_data["attachments"].get("nodes"):
        cleaned["attachments"] = [clean_attachment_data(attachment) for attachment in issue_data["attachments"]["nodes"]]
...
async def resolve_template_by_name(context: ToolContext, template_name: str, team_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if len(exact_matches) == 1:
        ...
        template_names = [f"{template['name']} ({template.get('team', {}).get('name', 'Global')})" 
