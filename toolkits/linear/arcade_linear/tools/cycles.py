...
async def list_cycles(
    ...
    team: Annotated[
        str,
        "Team name or ID to list cycles for (e.g. 'Frontend', 'Backend', 'Product')."
    ],
    active_only: Annotated[
        bool,
        "Whether to show only active cycles. Defaults to True."
    ] = True,
    limit: Annotated[
        int,
        "Maximum number of cycles to return. Min 1, max 50. Defaults to 10."
    ] = 10,
    ...
    cycles_response = await client.get_cycles(
        ...
        include_completed=not active_only
    )
...
async def get_cycle_issues(
    ...
    cycle: Annotated[
        str,
        "Cycle name, number, or ID to get issues for."
    ],
    team: Annotated[
        Optional[str],
    ...
    if cycle.startswith("cycle_"):  # Assume it's an ID
        ...
        if cycle_data:
            ...
            return {"error": f"Cycle not found: {cycle}. Please check the cycle name and try again."}
