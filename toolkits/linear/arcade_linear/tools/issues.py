...
from arcade_linear.constants import IssueSortField, SortDirection
...
async def search_issues(
    ...
    limit: Annotated[
        int,
        "Maximum number of issues to return. Min 1, max 100. Defaults to 50."
    ] = 50,
    ...
    This is the MAIN tool for finding issues in Linear. Use this tool for ANY issue search or filtering need.
    ...
    if created_after:
        if is_relative_date_range_term(created_after):
            range_start, range_end = parse_date_range_for_relative_terms(created_after)
    ...
    if created_before and created_before_date is None:
        if is_relative_date_range_term(created_before):
            range_start, range_end = parse_date_range_for_relative_terms(created_before)
            created_before_date = range_end
        else:
    ...
    if updated_after:
        if is_relative_date_range_term(updated_after):
            ...
            range_start, range_end = parse_date_range_for_relative_terms(updated_after)
    ...
    if updated_before and updated_before_date is None:
        if is_relative_date_range_term(updated_before):
            range_start, range_end = parse_date_range_for_relative_terms(updated_before)
            updated_before_date = range_end
        else:
        ...
        if team.startswith("team_"):  # Assume it's an ID
            ...
            if team_data:
                ...
                try:
                    ...
                    if available_teams:
                        team_names = [t["name"] for t in available_teams[:10]]  # Show up to 10 teams
                        team_list = "', '".join(team_names)
                        if len(available_teams) > 10:
                            return {"error": f"Team '{team}' not found. Available teams include: '{team_list}' and {len(available_teams) - 10} others. Use 'get_teams' tool to see all teams, then specify the correct team name."}
                        else:
                            return {"error": f"Team '{team}' not found. Available teams: '{team_list}'. Please specify one of these existing team names."}
                    else:
                        return {"error": f"Team '{team}' not found and no teams could be retrieved. Please check the team name or use 'get_teams' tool to see available teams."}
                except Exception:
                    return {"error": f"Team '{team}' not found. Please check the team name or use 'get_teams' tool to see available teams."}
...
async def get_issue(
    ...
    include_attachments: Annotated[
        bool,
        "Whether to include attachments in the response. Defaults to True."
    ] = True,
    ...
    include_children: Annotated[
        bool,
        "Whether to include sub-issues in the response. Defaults to True."
    ] = True,
    ...
    return {
        "issue": cleaned_issue,
        "retrieved_at": parse_date_string("now").isoformat() if parse_date_string("now") else None,
    }
...
async def update_issue(
    ...
    issue_id: Annotated[
        str,
        "The Linear issue ID or identifier (e.g. 'FE-123', 'API-456') to update."
    ],
    title: Annotated[
        Optional[str],
        "New title for the issue. Defaults to None (no change)."
    ] = None,
    description: Annotated[
        Optional[str],
        "New description for the issue. Defaults to None (no change)."
    ] = None,
    status: Annotated[
        Optional[str],
    ...
    if assignee is not None:
        if assignee.lower() == "me":
            ...
            if user_data:
                ...
                return {"error": f"User not found: {assignee}. Please check the username or email and try again."}
            ...
            if state_data:
                ...
                try:
                    ...
                    if available_states and available_states.get("nodes"):
                        ...
                        return {"error": f"Workflow state '{status}' not found for this team. Available statuses: {', '.join(state_names)}. You can also create a new workflow state using the 'create_workflow_state' tool if needed."}
                    else:
                        return {"error": f"Workflow state '{status}' not found for this team. Use 'get_workflow_states' to see available statuses or 'create_workflow_state' to create new ones."}
                except Exception:
    ...
    if cycle is not None:
        if cycle.lower() == "none":
            ...
            relative_keywords = [
                "current", "next", "previous", "last", "next week", "next month"
            ]
            is_relative = (
                cycle.lower() in relative_keywords or 
                cycle.lower().startswith("in ") or
                cycle.lower().startswith("in")
            )
            ...
            if cycle_data:
                ...
                if is_relative:
                    return {"error": f"No cycle found for '{cycle}'. This could mean no cycles are configured for the team, or no cycle matches the time period specified."}
                else:
                    return {"error": f"Cycle not found: {cycle}. Please check the cycle name/number and try again."}
    ...
    if ("stateId" in update_input and description is not None and status):
        ...
        closure_status_names = [
            ...
            "shipped", "released", "deployed", "merged", "fixed", "solved"
        ]
        
        is_closure_status = any(
            closure_name in status.lower() 
            for closure_name in closure_status_names
        )
    ...
    if result["success"]:
        ...
        if close_reason_comment:
            try:
                comment_result = await client.create_comment(issue_id, close_reason_comment)
...
async def create_issue(
    ...
    title: Annotated[
        str,
        "Title of the issue to create. Should be descriptive and concise."
    ],
    team: Annotated[
        str,
    ...
    parent_issue: Annotated[
        Optional[str],
        "Parent issue ID if this is a sub-issue. Defaults to None."
    ] = None,
    template: Annotated[
        Optional[str],
    ...
    estimate: Annotated[
        Optional[int],
        "Story point estimate (1-21). Defaults to None."
    ] = None,
    ...
    if parent_issue:
        ...
        if not parent_issue_data:
            return {"error": f"Parent issue not found: {parent_issue}. Please provide a valid issue identifier."}
    ...
    if parent_issue_data:
        ...
        if team.startswith("team_"):
            if team != team_id:
                return {"error": f"Sub-issue must be created in the same team as parent issue. Parent issue {parent_issue} is in team '{parent_team_name}'. Sub-issues cannot be in different teams."}
        else:
            ...
            if user_team_data and user_team_data["id"] != team_id:
                return {"error": f"Sub-issue must be created in the same team as parent issue. Parent issue {parent_issue} is in team '{parent_team_name}', but you specified team '{team}'. Sub-issues cannot be in different teams."}
    else:
    ...
    if template:
        if template.startswith("template_"):  # Assume it's an ID
            ...
            if template_data:
                ...
                return {"error": f"Template not found: {template}. Please check the template name and try again."}
    ...
    if cycle:
        if cycle.startswith("cycle_"):  # Assume it's an ID
            ...
            if cycle_data:
                ...
                if not status:
                    await _set_todo_status_for_sprint_creation(context, create_input, team_id)
            else:
                if is_relative:
                    return {"error": f"No cycle found for '{cycle}'. This could mean no cycles are configured for the team, or no cycle matches the time period specified."}
                else:
...
async def add_comment_to_issue(
    ...
    comment: Annotated[
        str,
        "The comment text to add to the issue. Supports markdown formatting."
    ],
...
async def get_templates(
    ...
    if not cleaned_templates:
        if team:
            response["message"] = f"No templates found for team '{team}'. Templates may not be set up for this team yet."
        else:
