...
def mock_context():
    ...
    context.secrets.append(
        ToolSecretItem(key="BRIGHTDATA_API_KEY", value=BRIGHTDATA_API_KEY)
    )
    context.secrets.append(
        ToolSecretItem(key="BRIGHTDATA_ZONE", value=BRIGHTDATA_ZONE)
    )
    return context
...
class TestExtractStructuredData:
    ...
    def test_extract_structured_data_invalid_source_type(self, mock_post):
        ...
        assert "Invalid source_type: invalid_source" in str(exc_info.value)
