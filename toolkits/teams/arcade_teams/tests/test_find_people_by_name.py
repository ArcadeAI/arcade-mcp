...
class TestFindPeopleByName:
    ...
    @pytest.mark.asyncio
    async def test_zero_matches(self, mock_context, mock_client, person_factory, response_factory):
        people = [
            person_factory(first_name="John", last_name="Smith"),
            person_factory(first_name="Jane", last_name="Foo"),
        ]
...
class TestFindUsersByName:
    ...
    async def test_zero_matches(
        self, mock_context, mock_client, person_factory, user_factory, response_factory
    ):
        users = [
            user_factory(first_name="John", last_name="Smith"),
            user_factory(first_name="Jane", last_name="Foo"),
...
class TestFindPeopleAndUsersByName:
    ...
    async def test_conflicting_exact_matches(
        ...
        assert "John Smith" in error.value.message
        assert smith_person_match in error.value.additional_prompt_content
        assert smith_user_match in error.value.additional_prompt_content

        assert "Foo" not in error.value.message
        assert "Foo" not in error.value.additional_prompt_content
        assert "Jane" not in error.value.message
        assert "Jane" not in error.value.additional_prompt_content
    ...
    async def test_unique_exact_person_match_and_partial_user_match(
        ...
        assert "Foo Bar" in error.value.message
        assert foo_baron_match in error.value.additional_prompt_content

        assert "John Smith" not in error.value.message
        assert "John Smith" not in error.value.additional_prompt_content
        assert "Jane Foo" not in error.value.message
        assert "Jane Foo" not in error.value.additional_prompt_content
        assert "Hello World" not in error.value.message
        assert "Hello World" not in error.value.additional_prompt_content
    ...
    async def test_unique_exact_user_match_and_partial_person_match(
        ...
        assert "John Smith" in error.value.message
        assert john_smith_match in error.value.additional_prompt_content

        assert "Foo Bar" not in error.value.message
        assert "Foo Bar" not in error.value.additional_prompt_content
        assert "Jane Foo" not in error.value.message
        assert "Jane Foo" not in error.value.additional_prompt_content
        assert "Hello World" not in error.value.message
        assert "Hello World" not in error.value.additional_prompt_content
